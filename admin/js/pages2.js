/* ---------------------------------------------------------
   PAGES2.JS — remaining admin pages (certificates, QR codes,
   email, push, export, audit log, users, permissions, roles,
   settings) — continues js/pages.js
--------------------------------------------------------- */

function certTypeLabel(type) {
  const lang = state.lang;
  return L(T[`certType_${type}`] || T.certType_participation, lang);
}

/* simple field setters — text inputs update state without a full
   re-render (so focus isn't lost while typing); selects/dates re-render */
function setCertMember(v) { db.ui.certMember = v; }
function setCertTitle(v) { db.ui.certTitle = v; }
function setCertType(v) { db.ui.certType = v; render(); }
function setCertIssueDate(v) { db.ui.certIssueDate = v; render(); }
...
function pageCertificates() {
  const lang = state.lang;
  const { certMember, certTitle, certType, certIssueDate, justIssued, copied } = db.ui;
  return `
    <div>
      ${pageHeader(T.m_certificates)}
      <div class="card p-5 max-w-xl mb-6">
        ...
        <input class="input-field mb-3" placeholder="${L(T.certTitlePlaceholder, lang)}" value="${certTitle}" oninput="setCertTitle(this.value)" />

        <div class="grid sm:grid-cols-2 gap-3 mb-4">
          <div>
            <label class="text-sm text-rope block mb-1">${L(T.certTypeLabel, lang)}</label>
            <select class="input-field" onchange="setCertType(this.value)">
              ${["participation","completion","appreciation","badge"].map(t => `<option value="${t}" ${certType === t ? "selected" : ""}>${certTypeLabel(t)}</option>`).join("")}
            </select>
          </div>
          <div>
            <label class="text-sm text-rope block mb-1">${L(T.certDateLabel, lang)}</label>
            <input type="date" class="input-field" value="${certIssueDate}" oninput="setCertIssueDate(this.value)" />
          </div>
        </div>

        <button class="btn-primary flex items-center gap-2" ${(!certMember || !certTitle.trim() || !certIssueDate) ? "disabled" : ""} onclick="issueCertificate()">
          ${icon("award", 'style="width:16px;height:16px"')} ${L(T.certGenerateBtn, lang)}
        </button>

        ${justIssued ? `
          <div class="mt-4 p-4 border-2 border-dashed border-gold rounded-lg text-center bg-forest">
            ${icon("award", 'style="width:26px;height:26px" class="text-gold mx-auto mb-2"')}
            <div class="text-cream ap-display font-bold">${justIssued.member}</div>
            <div class="text-cream text-xs opacity-75 mb-2">${L(T.certGenerated, lang)}</div>
            <div class="flex items-center justify-center gap-2 text-cream text-xs">
              <span class="opacity-75">${L(T.certVerifyCode, lang)}:</span>
              <code class="bg-forest-mid px-2 py-0.5 rounded">${justIssued.code}</code>
              <button onclick="copyCertCode('${justIssued.code}')" class="icon-btn !w-7 !h-7">${icon("copy", 'style="width:12px;height:12px"')}</button>
            </div>
            ${copied === justIssued.code ? `<div class="text-gold text-xs mt-1">${L(T.copied, lang)}</div>` : ""}
          </div>` : ""}
      </div>

      ${pageHeader(T.certIssuedList)}
      <div class="card overflow-hidden overflow-x-auto">
        <table class="w-full text-sm">
          <thead class="bg-canvas text-forest"><tr>
            <th class="text-left p-3">${L(T.colName, lang)}</th>
            <th class="text-left p-3">${L(T.certTitleLabel, lang)}</th>
            <th class="text-left p-3">${L(T.colType, lang)}</th>
            <th class="text-left p-3">${L(T.colDate, lang)}</th>
            <th class="text-left p-3">${L(T.colVerifyCode, lang)}</th>
            <th class="text-left p-3">${L(T.colAction, lang)}</th>
          </tr></thead>
          <tbody>
            ${db.certificates.map(c => `
              <tr class="border-t border-rope border-opacity-20">
                <td class="p-3 text-forest">${c.member}</td>
                <td class="p-3 text-rope">${c.title}</td>
                <td class="p-3 text-rope">${certTypeLabel(c.type)}</td>
                <td class="p-3 text-rope">${c.date}</td>
...
function pageQrCodes() {
  const lang = state.lang;
  return `
    <div>
      ${pageHeader(T.m_qrcodes, T.qrSub)}
      <div class="card p-4 mb-6">
        ...
        <button class="btn-primary mt-4 flex items-center gap-2" ${db.qrSelected.length ? "" : "disabled"} onclick="generateQrSelected()">${icon("qr-code", 'style="width:16px;height:16px"')} ${L(T.qrGenerateSelected, lang)}</button>
      </div>

      ${pageHeader(T.qrCardsTitle)}
      ${db.qrGenerated.length === 0 ? `<div class="text-rope text-sm">${L(T.qrEmpty, lang)}</div>` : `
          ...
          ${db.qrGenerated.map(m => `
            <div class="card p-4 flex flex-col items-center text-center gap-2">
              ...
              <div class="flex gap-2 mt-1">
                ...
                <button class="btn-outline text-xs flex items-center gap-1" onclick="window.print()">${icon("printer", 'style="width:12px;height:12px"')} ${L(T.print, lang)}</button>
              </div>
            </div>`).join("")}
        </div>`}
      <p class="text-rope text-xs mt-4 italic">${L(T.qrPreviewNote, lang)}</p>
    </div>`;
}
...
const PAGE_RENDERERS = {
  ...
  events: pageEvents, gallery: pageGallery, notices: pageNotices, certificates: pageCertificates,
  qrcodes: pageQrCodes, email: pageEmail, pushsettings: pagePushSettings, exporttools: pageExportTools,
